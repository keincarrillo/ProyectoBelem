...
#include <sys/wait.h> // manejo de procesos
...
int dividePipes(char *input, char *cmds[]) { 
    int i = 0;
...
void manejoRedirecciones(char **args) {
    for (int i = 0; args[i] != NULL; i++) { // Recorre todos los argumentos hasta encontrar NULL
        if (strcmp(args[i], "<") == 0) { // Compara la cadena si se trata de "<" y devuelve 0 si son iguales
            ...
            if (fd < 0) { 
                perror(ROJO "Error al abrir el archivo de entrada" RESET);
                exit(1);
            }
            ...
            if (fd < 0) {
                perror(ROJO "Error al abrir el archivo de salida" RESET);
                exit(1);
            }
...
void ejecutaComando(char *input) {
    char *cmds[MAX_CMDS]; // Arreglo para almacenar comandos separados por tuberias
    int num_cmds = dividePipes(input, cmds); // Divide la entrada por |
    ...
    for (int i = 0; i < num_cmds - 1; i++) { // Crea una tuberia por cada comando
        if (pipe(pipefds + 2 * i) < 0) {  // Crea la tuberia con su descriptores de archivo (pipefds + 2 * 0) = pipefds[0], pipefds[1], (pipefds + 2 * 1) = pipefds[2] , pipefds[3]
            perror(ROJO "Error al crear la tubería" RESET);
            exit(1);
        }
    }
    ...
    for (int i = 0; i < num_cmds; i++) { // Se crea un proceso por cada comando
        ...
        if (pid == 0) { // Proceso hijo
            ...
            fprintf(stderr, ROJO "%s: no se encontró la orden\n" RESET, args[0]);
            exit(1);
        } else if (pid < 0) {
            fprintf(stderr, ROJO ": Error al crear el proceso \n" RESET);
            exit(1);
        }
    }
...
int main() {
    ...
    char input[MAX_INPUT]; // Cadena de entrada 
    const char *username = getenv("USER"); // Variable de entorno del usuario

    while (1) {
        printf(AMARILLO "%s> " RESET, username); // Imprime el prompt
        fflush(stdout); // Limpia el buffer de entrada
        if (fgets(input, MAX_INPUT, stdin) == NULL) { // Almacena la cadena de entrada
            break;
        }
        if (strcmp(input, "exit\n") == 0) { // Compara la cadena input con exit\n para salir del programa
            break;
        }
        ejecutaComando(input); // Procede a ejecutar el comando 
    }
